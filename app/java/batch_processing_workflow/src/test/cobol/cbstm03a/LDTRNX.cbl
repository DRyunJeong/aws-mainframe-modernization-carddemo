       ...
       file-control.
           select s assign to SRCTRNX organization line sequential
               file status f1.
           select d assign to TRNXFILE organization indexed access dynamic
               record key k file status f2.
       data division.
       file section.
       fd s. 01 s-rec pic x(350).
       fd d. 01 d-rec. 05 k pic x(32). 05 filler pic x(318).
